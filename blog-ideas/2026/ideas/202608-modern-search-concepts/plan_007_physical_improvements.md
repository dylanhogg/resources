...
| L5  | Leave the logical `legendNote` sentence alone                | Phase 4 rewrote the _physical_ legend because assigning `confidence` made "Retrieval routing owns every request gate" false there (F24). Logical declares `confidence:"understand"` and gates nothing on it, so the original sentence is still true at that level.                                                           | None — record the asymmetry rather than syncing the two strings.                        |
