...
### Indirect prompt injection and RAG contamination
...
### Data leakage and context exposure
...
For structured tasks, use schema-constrained output where possible. Validate types, enums, ranges, required fields, and unsupported values. Normalize before storing or executing. Treat malformed output as a system event, not as something to patch over silently.
...
Add schemas, validators, and normalization. Any model output that affects downstream logic should have a contract.
...
Track safety metrics alongside task success, latency, and cost. If safety is not measured, it will lose to easier-to-optimize product metrics.
